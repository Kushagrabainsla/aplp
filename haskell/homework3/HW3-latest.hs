...
endIf :: Parser ()
endIf = keyword "endif" <|> keyword "end"

endWhile :: Parser ()
endWhile = keyword "endwhile" <|> keyword "end"

reservedOp :: String -> (a -> a -> a) -> Parser (a -> a -> a)
reservedOp op constructor = keyword op *> return constructor

reservedNames :: [String]
reservedNames = ["if", "then", "else", "end", "endif", "while", "do", "endwhile", "true", "false", "skip", "and", "or", "not"]
...
main = do
  ...
  case args of
    ...
    _          -> putStrLn "Usage: runhaskell HW3-1.hs <filename.imp>"
